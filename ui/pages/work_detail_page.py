...
class WorkDetailPage(QWidget):
    ...
    def set_work(self, work):
        ...
        root.addWidget(self._grid_section(
            "Characters", get_characters(self._value("id")), CharacterCard,
            self.character_selected, 4
        ))
        root.addWidget(self._grid_section(
            "Staff", get_staff(self._value("id")), PersonCard,
            self.person_selected, 4
        ))
        ...
        self.setStyleSheet(f"""
            QPushButton#back {{
                background: transparent; border: 0; color: {COLORS['secondary']};
                padding: 5px 0; font-weight: 750;
            }}
            ...
            QFrame#progressTrack {{
                background: {COLORS['border']}; border: 0; border-radius: 5px;
            }}
            QFrame#progressFill {{
                background: {COLORS['accent']}; border: 0; border-radius: 5px;
            }}
            QSpinBox#episodeCounter {{
                background: {COLORS['background_alt']}; color: {COLORS['primary']};
                border: 1px solid {COLORS['border_hover']}; border-radius: 10px;
                padding: 8px 12px; font-size: 18px; font-weight: 850;
                min-width: 92px;
            }}
            QSpinBox#episodeCounter:focus {{ border-color: {COLORS['accent']}; }}
            QSpinBox#episodeCounter::up-button, QSpinBox#episodeCounter::down-button {{
                width: 0; height: 0; border: 0;
            }}
            QPushButton#counterButton {{
                background: {COLORS['surface_alt']}; color: {COLORS['primary']};
                border: 1px solid {COLORS['border']}; border-radius: 10px;
                font-size: 18px; font-weight: 850; min-width: 38px; min-height: 38px;
            }}
    ...
    def _hero(self):
        ...
        box = QHBoxLayout(hero)
        box.setContentsMargins(24, 24, 28, 24)
        box.setSpacing(30)

        cover = QLabel()
        cover.setFixedSize(235, 335)
        ...
        if path:
            ...
            if not pix.isNull():
                cover.setPixmap(self._cropped_cover(pix, cover.size(), 14))
        if cover.pixmap() is None or cover.pixmap().isNull():
            cover.setText("NO COVER")
            cover.setStyleSheet(
                f"color:{COLORS['muted']}; background:{COLORS['background_alt']}; border-radius:14px;"
            )
        ...
        title.setStyleSheet(
            f"font-size:34px;font-weight:850;color:{COLORS['primary']};letter-spacing:-1px;"
        )
        info.addWidget(title)

        alt = self._value("native") or self._value("title_native") or ""
        if alt:
            native = QLabel(str(alt))
            native.setStyleSheet(muted_label_stylesheet())
            info.addWidget(native)
        ...
        if meta:
            ...
            metadata.setStyleSheet(f"color:{COLORS['secondary']};font-size:13px;")
        ...
        info.addSpacing(10)
        progress_title = QLabel("Episode progress")
        progress_title.setStyleSheet(
            f"color:{COLORS['secondary']};font-size:12px;font-weight:800;"
        )
        info.addWidget(progress_title)
        ...
        counter = QSpinBox()
        counter.setObjectName("episodeCounter")
        total = max(0, int(self._value("episodes") or 0))
        current = max(0, int(self._value("progress_episodes") or 0))
        counter.setRange(0, total if total else 99999)
        counter.setValue(min(current, counter.maximum()))
        counter.setAlignment(Qt.AlignCenter)
        counter.setButtonSymbols(QSpinBox.NoButtons)
        minus.clicked.connect(lambda: counter.setValue(counter.value() - 1))
        plus.clicked.connect(lambda: counter.setValue(counter.value() + 1))
        counter.valueChanged.connect(self._progress_counter_changed)
        progress_row.addWidget(minus)
        progress_row.addWidget(counter)
        progress_row.addWidget(plus)
        progress_row.addStretch()
        info.addLayout(progress_row)

        progress_track = QFrame()
        progress_track.setObjectName("progressTrack")
        progress_track.setFixedHeight(8)
        progress_fill = QFrame(progress_track)
        progress_fill.setObjectName("progressFill")
        ratio = (current / total) if total else 0
        progress_fill.setGeometry(0, 0, round(420 * min(1.0, ratio)), 8)
        info.addWidget(progress_track)
        progress_count = QLabel(
            f"{current} of {total} episodes" if total else "Episode count unavailable"
        )
        progress_count.setStyleSheet(muted_label_stylesheet())
        info.addWidget(progress_count)
        info.addStretch()

        self._episode_counter = counter
        self._progress_fill = progress_fill
        self._progress_track = progress_track
    ...
    def _description(self):
        ...
        header.setStyleSheet(f"font-size:17px;font-weight:800;color:{COLORS['primary']};")
        lay.addWidget(header)
        label = QLabel(self._value("description") or "No description saved locally.")
        label.setWordWrap(True)
        label.setTextFormat(Qt.PlainText)
        label.setStyleSheet(f"color:{COLORS['secondary']};font-size:14px;")
        lay.addWidget(label)
        return frame
    ...
    def _episodes_section(self):
        ...
        title.setStyleSheet(f"font-size:17px;font-weight:800;color:{COLORS['primary']};")
        header.addWidget(title)
        header.addStretch()
        watched = sum(1 for ep in episodes if ep["watched"])
        total = len(episodes)
        count = QLabel(f"{watched} / {total} watched" if total else "No episode list saved")
        ...
        if not episodes:
            x = QLabel(
                "Use the episode counter above to track progress. Detailed episode data will appear here when saved locally."
            )
        ...
        for ep in episodes:
            ...
            num.setStyleSheet(f"color:{COLORS['accent']};font-weight:850;")
            r.addWidget(num)
            ep_title = QLabel(ep["title"] or "Episode")
            ep_title.setStyleSheet(f"color:{COLORS['primary']};font-weight:650;")
            r.addWidget(ep_title, 1)
            date = QLabel(str(ep["air_date"] or ""))
            date.setStyleSheet(muted_label_stylesheet())
            r.addWidget(date)
            cb.toggled.connect(
                lambda checked, n=ep["episode_number"]: self._episode_toggled(n, checked)
            )
            lay.addWidget(row)
        return frame

    def _episode_toggled(self, number, checked):
        set_episode_watched(self._value("id"), number, checked)
        refreshed = get_work(self._value("id"))
        self.set_work(refreshed or self.work)
    ...
    def _relations(self):
        return self._grid_section(
            "Relations", get_relations(self._value("id")), RelationCard,
            self.relation_selected, 2
        )
