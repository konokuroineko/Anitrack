...
from PySide6.QtWidgets import QCheckBox, QColorDialog, QComboBox, QFrame, QHBoxLayout, QLabel, QPushButton, QSlider, QVBoxLayout, QWidget

from database import get_all_library
from ui.preferences import defaults, get, reset, set_value
from ui.theme import COLORS, SPACING, refresh_theme, application_stylesheet
...
class SettingsPage(QWidget):
    ...
    def _section(self, name, widgets):
        panel = QFrame(); panel.setStyleSheet(f"QFrame{{background:{COLORS['surface']};border:1px solid {COLORS['border']};border-radius:{get('corner_radius')}px;}}")
        box = QVBoxLayout(panel); box.setContentsMargins(22, 18, 22, 12); box.setSpacing(0)
        heading = QLabel(name.upper()); heading.setStyleSheet(f"color:{COLORS['accent']};font-size:10px;font-weight:900;letter-spacing:1.4px;padding-bottom:8px;")
        box.addWidget(heading)
        for widget in widgets: box.addWidget(widget)
        return panel

    def _row(self, name, description, control):
        row = QHBoxLayout(); row.setContentsMargins(0, 11, 0, 11); row.setSpacing(18)
        text = QVBoxLayout(); text.setSpacing(2)
        label = QLabel(name); label.setStyleSheet(f"font-weight:700;color:{COLORS['primary']};")
        desc = QLabel(description); desc.setWordWrap(True); desc.setStyleSheet(f"color:{COLORS['muted']};font-size:11px;")
        text.addWidget(label); text.addWidget(desc); row.addLayout(text, 1); row.addWidget(control, 0, Qt.AlignRight | Qt.AlignVCenter)
        wrapper = QWidget(); wrapper.setLayout(row); return wrapper

    def _color_row(self, name, description, key):
        button = QPushButton(); button.setFixedWidth(110); button.setCursor(Qt.PointingHandCursor)
    ...
    def _paint_color_button(self, button, value):
        ...
        button.setStyleSheet(f"QPushButton{{background:{value};color:#101216;border:1px solid {COLORS['border']};border-radius:9px;padding:8px 12px;font-weight:800;}}QPushButton:hover{{border-color:{COLORS['primary']};}}")
    ...
    def _combo_row(self, name, description, key, options):
        combo = QComboBox(); combo.setMinimumWidth(150)
        for label, value in options: combo.addItem(label, value)
        current = get(key)
        for i in range(combo.count()):
            if combo.itemData(i) == current: combo.setCurrentIndex(i); break
    ...
    def _check_row(self, name, description, key):
        check = QCheckBox(); check.setChecked(get(key)); check.stateChanged.connect(lambda state, k=key: self._changed(k, bool(state)))
        return self._row(name, description, check)

    def _info_row(self, name, value):
        value_label = QLabel(value); value_label.setStyleSheet(f"color:{COLORS['secondary']};")
    ...
    def _reset(self):
        reset()
        for key, value in defaults().items(): set_value(key, value)
        self._apply()
