...
def save_anime(anime):
    ...
    for edge in (anime.get("relations") or {}).get("edges") or []:
        ...
        connection.execute("INSERT OR REPLACE INTO work_relations (source_id, target_id, relation_type) VALUES (?, ?, ?)",
                           (anime["id"], target_id, edge.get("relationType")))
    connection.commit()
    connection.close()
...
def get_all_library():
    ...
    results = connection.execute("""
        SELECT works.*, user_library.status, user_library.progress_episodes, user_library.progress_chapters,
               user_library.rating, user_library.notes, user_library.added_date, user_library.updated_date
        FROM user_library JOIN works ON works.id = user_library.work_id ORDER BY works.title
    """).fetchall()
    connection.close()
    return results
...
def update_library_progress(work_id, episodes=None, chapters=None):
    ...
    if episodes is not None:
        connection.execute("UPDATE user_library SET progress_episodes = ?, updated_date = CURRENT_TIMESTAMP WHERE work_id = ?",
                           (episodes, work_id))
    if chapters is not None:
        connection.execute("UPDATE user_library SET progress_chapters = ?, updated_date = CURRENT_TIMESTAMP WHERE work_id = ?",
                           (chapters, work_id))
    connection.commit()
    connection.close()
