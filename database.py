...
def save_characters(work_id, characters):
    ...
    for edge in characters or []:
        ...
        connection.execute(
            "INSERT OR IGNORE INTO work_characters (work_id, character_id) VALUES (?, ?)",
...
def set_episode_watched(work_id, episode_number, watched):
    connection = get_connection()
    connection.execute(
        "UPDATE episodes SET watched = ? WHERE work_id = ? AND episode_number = ?",
    ...
    if library_row:
        if total and watched_count >= total:
            status = "Completed"
...
def save_anime(anime):
    ...
    """, (
        ...
        anime.get("episodes"), anime.get("averageScore"), start_year,
        cover_image.get("large"), anime.get("format"), anime.get("chapters"),
        anime.get("volumes"), anime.get("source"), (anime.get("endDate") or {}).get("year"),
        anime.get("duration"),
    ))
    ...
    for edge in (anime.get("relations") or {}).get("edges") or []:
        ...
        if target_title:
            ...
            connection.execute("""
                INSERT OR IGNORE INTO works (id, title, type, format, cover_url)
                VALUES (?, ?, ?, ?, ?)
            """, (
        ...
        connection.execute("""
            INSERT OR REPLACE INTO work_relations (source_id, target_id, relation_type)
            VALUES (?, ?, ?)
...
def get_work(work_id):
    connection = get_connection()
    result = connection.execute("SELECT * FROM works WHERE id = ?", (work_id,)).fetchone()
    connection.close()
    return result
...
def get_relations(work_id):
    connection = get_connection()
    results = connection.execute("""
        SELECT work_relations.*, works.title, works.format, works.type,
               works.cover_url, works.cover_path
        FROM work_relations
...
def get_library_by_status(status):
    ...
    results = connection.execute("""
        ...
        FROM user_library JOIN works ON works.id = user_library.work_id
        WHERE user_library.status = ? ORDER BY works.title
    """, (status,)).fetchall()
    connection.close()
    return results
...
def update_library_progress(work_id, episodes=None, chapters=None):
    ...
    if episodes is not None:
        connection.execute(
            "UPDATE user_library SET progress_episodes = ?, updated_date = CURRENT_TIMESTAMP WHERE work_id = ?",
            (episodes, work_id),
        )
    if chapters is not None:
        connection.execute(
            "UPDATE user_library SET progress_chapters = ?, updated_date = CURRENT_TIMESTAMP WHERE work_id = ?",
            (chapters, work_id),
        )
    connection.commit()
    connection.close()
